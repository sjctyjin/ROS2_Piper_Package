...
class PiperRosNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter('arm_prefix', '')  # 新增: 關節前綴參數
        ...
        self.get_logger().info(f"arm_prefix is '{self.arm_prefix}'")  # 打印前綴信息
        
        # 使用前綴定義關節名稱
        self.joint_names = [
            f'{self.arm_prefix}joint1', 
            f'{self.arm_prefix}joint2', 
            f'{self.arm_prefix}joint3', 
            f'{self.arm_prefix}joint4', 
            f'{self.arm_prefix}joint5', 
            f'{self.arm_prefix}joint6', 
            f'{self.arm_prefix}gripper'
        ]
        pub_prefix = self.arm_prefix.split('_')[0]
        # Publishers - 修改話題使用指定的命名空間
        self.joint_pub = self.create_publisher(JointState, f'{pub_prefix}/joint_states_single', 1)  # 原始話題保留，用於相容
        self.joint_ctrl_pub = self.create_publisher(JointState, f'{pub_prefix}/joint_ctrl', 1)
        self.arm_status_pub = self.create_publisher(PiperStatusMsg, f'{pub_prefix}/arm_status', 1)  # 添加前綴
        self.end_pose_pub = self.create_publisher(Pose, f'{pub_prefix}/end_pose', 1)  # 添加前綴
        
        # Service
        self.motor_srv = self.create_service(Enable, f'{pub_prefix}/enable_srv', self.handle_enable_service)  # 添加前綴
        
        # Joint
        self.joint_states = JointState()
        self.joint_states.name = self.joint_names  # 使用前綴關節名稱
        ...
        self.joint_ctrl.name = self.joint_names  # 使用前綴關節名稱
        ...
        # Start subscription thread - 修改話題使用前綴
        self.create_subscription(PosCmd, f'{pub_prefix}/pos_cmd', self.pos_callback, 1)  # 添加前綴
        self.create_subscription(JointState, 'joint_ctrl_single', self.joint_callback, 1)  # 這個用於接收外部命令，不修改
        self.create_subscription(Bool, f'{pub_prefix}/enable_flag', self.enable_callback, 1)  # 添加前綴
    ...
    def joint_callback(self, joint_data):
        ...
        # 移除前綴以進行關節匹配
        prefix_len = len(self.arm_prefix)
        
        # 遍歷joint_data.name來映射位置
        for idx, joint_name in enumerate(joint_data.name):
            # 檢查關節名稱是否與目前節點的前綴匹配
            # 如果匹配，或者沒有前綴（向後兼容），則處理該關節
            if joint_name.startswith(self.arm_prefix) or self.arm_prefix == '':
                # 移除前綴以獲取基本關節名稱
                base_joint_name = joint_name
                if self.arm_prefix and joint_name.startswith(self.arm_prefix):
                    base_joint_name = joint_name[prefix_len:]
                    
                # 存儲基本關節名稱和位置
                joint_positions[base_joint_name] = round(joint_data.position[idx] * factor)
        ...
        if len(joint_data.position) >= 7:
            ...
            # 嘗試找到夾爪關節的實際索引
            for idx, joint_name in enumerate(joint_data.name):
                if (self.arm_prefix and joint_name == f'{self.arm_prefix}gripper') or \
                   (not self.arm_prefix and joint_name == 'gripper'):
                    gripper_idx = idx
                    break
            
            if gripper_idx < len(joint_data.position):
        ...
        if self.GetEnableFlag():
            ...
            if self.gripper_exist:
                ...
                if len(joint_data.effort) >= 7:
                    ...
                    if gripper_idx < len(joint_data.effort):
                        gripper_effort = clip(joint_data.effort[gripper_idx], 0.5, 3)
