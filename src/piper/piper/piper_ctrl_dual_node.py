...
class PiperRosNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.joint_pub = self.create_publisher(JointState, 'joint_states_single', 1)  # 原始話題保留，用於相容
        self.joint_ctrl_pub = self.create_publisher(JointState, 'joint_ctrl', 1)
        self.arm_status_pub = self.create_publisher(PiperStatusMsg, f'{self.arm_prefix}arm_status', 1)  # 添加前綴
        self.end_pose_pub = self.create_publisher(Pose, f'{self.arm_prefix}end_pose', 1)  # 添加前綴
        
        # Service
        self.motor_srv = self.create_service(Enable, f'{self.arm_prefix}enable_srv', self.handle_enable_service)  # 添加前綴
        ...
        self.create_subscription(PosCmd, f'{self.arm_prefix}pos_cmd', self.pos_callback, 1)  # 添加前綴
        self.create_subscription(JointState, 'joint_ctrl_single', self.joint_callback, 1)  # 這個用於接收外部命令，不修改
        self.create_subscription(Bool, f'{self.arm_prefix}enable_flag', self.enable_callback, 1)  # 添加前綴
