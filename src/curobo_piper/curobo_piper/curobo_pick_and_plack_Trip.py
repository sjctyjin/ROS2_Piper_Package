...
class EnhancedDualArmTracker(Node):
    ...
    def __init__(self):
        ...
        # 創建夾爪測試定時器（1Hz）
        # self.create_timer(1.0, self.test_gripper_operation)
    ...
    def setup_original_parameters(self):
        ...
        # 左右手臂配置
        self.declare_parameter('left_arm_config', 'trip_piper_left.yml')
        self.declare_parameter('right_arm_config', 'trip_piper_right.yml')
        self.declare_parameter('mpc_max_iters', 30)
        
        # 協作策略參數
        self.declare_parameter('cooperation_mode', 'offset')
        self.declare_parameter('arm1_offset_x', 0.12)
        self.declare_parameter('arm1_offset_y', 0.1)
        self.declare_parameter('arm1_offset_z', 0.15)
        self.declare_parameter('arm2_offset_x', 0.12)
        self.declare_parameter('arm2_offset_y', 0.1)
        self.declare_parameter('arm2_offset_z', 0.1)
    ...
    def setup_fine_positioning_parameters(self):
        """設置精細定位相關參數"""
        # 精細定位觸發參數
        self.declare_parameter('fine_trigger_distance', 0.20)     # 觸發精細定位的距離閾值
        self.declare_parameter('fine_positioning_timeout', 6.0)  # 精細定位超時時間
        self.declare_parameter('fine_mpc_steps', 150)            # 精細定位MPC最大步數
    ...
    def setup_independent_mpc_controllers(self):
        ...
        try:
            ...
            left_mpc_config = MpcSolverConfig.load_from_robot_config(
                ...
                self_collision_check=True, collision_checker_type=None,
                collision_cache={"obb": 10, "mesh": 5},
                use_mppi=True, use_lbfgs=False, use_es=False,
                store_rollouts=False, step_dt=0.05,
            )
    ...
    def get_fine_target_tf(self, target_frame):
        ...
        try:
            ...
            pose = Pose.from_list([
                position[0]+0.03, position[1], position[2],
                orientation[0], orientation[1], orientation[2],orientation[3]
            ])

            if target_frame == 'cam2_object_in_base':
                pose = Pose.from_list([
                    position[0]+0.1, position[1], position[2]+0.1,
                    orientation[0], orientation[1], orientation[2],orientation[3] 
                ])
    ...
    def update_fine_targets(self):
        """更新精細目標座標"""
        # 更新cam1目標 (左臂)
        cam1_pose = self.get_fine_target_tf('cam1_object_in_base')
        if cam1_pose is not None:
            self.left_fine_target_pose = cam1_pose
            
        # 更新cam2目標 (右臂)
        cam2_pose = self.get_fine_target_tf('cam2_object_in_base')
    ...
    def control_fine_positioning_mpc(self, mpc_controller, current_positions, target_pose, arm_name, gripper_value=None):
        """精細定位MPC控制（限定步數）"""
        self.get_logger().info(f"{arm_name} 精細定位開始，執行了 ")
            ...
            while step_count < max_steps:
                ...
                if result.metrics.feasible.item():
                    ...
                    if len(expected_joints) <= 6:
                        ...
                        if arm_name == 'arm1':
                            arm1_indices = [i for i, name in enumerate(expected_joints) if name.startswith('arm1')]
                            target_arm_positions = [new_full_positions[i] for i in arm1_indices[:6]] if len(arm1_indices) >= 6 else new_full_positions[:6]
                        else:
                            arm2_indices = [i for i, name in enumerate(expected_joints) if name.startswith('arm2')]
                            target_arm_positions = [new_full_positions[i] for i in arm2_indices[:6]] if len(arm2_indices) >= 6 else new_full_positions[6:12] if len(new_full_positions) >= 12 else new_full_positions[:6]
                    ...
                    if gripper_value is not None:
                        self.publish_joint_commands_with_gripper(target_arm_positions, arm_name, gripper_value)
                    else:
                        self.publish_joint_commands(target_arm_positions, arm_name)
                    ...
                    time.sleep(0.02)  # 控制頻率
                else:
                    self.get_logger().warning(f"{arm_name} 精細定位MPC步驟 {step_count} 不可行")
                    ckeck_step_set +=1
                    if ckeck_step_set > 20:
                        self.get_logger().error(f"{arm_name} 精細定位MPC步驟 {step_count} 連續不可行，退出循環")
                        self.fine_positioning_steps[arm_name] = max_steps
                        break
                    # self.get_logger().info(f"{arm_name} 精細定位MPC步
                    # break  # 如果不可行，則退出循環
                
                step_count += 1
            
            self.get_logger().info(f"{arm_name} 精細定位完成，執行了 {step_count} 步")
            return True
            ...
            try:
                ...
                if tf_result is not None:
                    ...
                    if (self.target_position is None or 
                        ...
                        r_z45 = Rotation_R.from_euler('y', 60, degrees=True)
    ...
    def handle_fine_positioning_mode(self):
        ...
        # 檢查精細定位是否完成或超時（這段只在 started 時才判斷）
        ...
        #     # 檢查是否完成
        #     # if (self.fine_positioning_steps['arm1'] >= self.fine_mpc_steps*0.6 and 
        #     #     self.fine_positioning_steps['arm2'] >= self.fine_mpc_steps*0.6):

        #     #     # 先執行夾爪操作
        #     #     arm1_success = self.operate_gripper('arm1', self.gripper_close_value)
        #     #     arm2_success = self.operate_gripper('arm2', self.gripper_close_value)
        #     #     # 等待並檢查夾爪狀態
        #     #     # time.sleep(2.0)  # 給夾爪一些時間動作
        #     #     # if arm1_success and arm2_success:
        #     #     #     # 檢查兩個夾爪的狀態
        #     #     #     if self.arm1_gripper_status and self.arm2_gripper_status:
        #     #     #         self.get_logger().info("✅ 雙臂夾取確認成功")
        #     #     #         # self.current_mode = OperationMode.TRANSPORT
        #     #     #         # self.current_stage = HarvestStage.TRANSPORT_RELEASE
        #     #     #         self.reset_to_tracking_mode()
        #     #     #     else:
        #     #     #         self.get_logger().error(f"❌ 夾取失敗 (ARM1: {'✓' if self.arm1_gripper_status else '✗'}, ARM2: {'✓' if self.arm2_gripper_status else '✗'})")
        #     #     #         self.reset_to_tracking_mode()
        ...
        #         for i in range(6):
        #             k += 1
        #             self.get_logger().info(f"檢查夾取狀態{k}")
        #             time.sleep(1)
                

        #         # # 等待並多次檢查夾爪狀態
        #         # max_check_attempts = 10  # 增加檢查次數
        #         # check_interval = 0.5
        #         # arm1_status = False
        #         # arm2_status = False
        ...
        #         #     # 檢查夾爪狀態
        #         #     if (hasattr(self, 'arm1_gripper_status') and 
        #         #         hasattr(self, 'arm2_gripper_status')):
        #         #         arm1_status = self.arm1_gripper_status
        #         #         arm2_status = self.arm2_gripper_status
                        
        #         #         # 如果兩個夾爪都確認狀態，可以提前退出
        #         #         if arm1_status and arm2_status:
        #         #             self.get_logger().info(f"✅ 夾取確認成功 (嘗試 {attempt + 1}/{max_check_attempts})")
        #         #             break
                            
        #         #     self.get_logger().info(f"檢查夾取狀態 ({attempt + 1}/{max_check_attempts})")
        ...
        #         #     self.reset_to_tracking_mode()
        #         # else:
        #         #     self.get_logger().error(f"❌ 夾取失敗 (ARM1: {'✓' if arm1_status else '✗'}, ARM2: {'✓' if arm2_status else '✗'})")
        #         #     # 夾取失敗時打開夾爪
        #         #     self.operate_gripper('arm1', self.gripper_open_value)
        #         #     self.operate_gripper('arm2', self.gripper_open_value)
        #         #     time.sleep(1.0)
        ...
        #         self.get_logger().warning("⏰ 精細定位執行超時，返回跟隨control_fine_positioning_mpc模式")
        #         self.reset_to_tracking_mode()
        if hasattr(self, 'fine_positioning_started') and self.fine_positioning_started:
            elapsed_time = time.time() - self.fine_positioning_start_time
            ...
            if (not self.arm1_mpc_running and not self.arm2_mpc_running and  
                # self.arm1_mpc_completed and  self.arm2_mpc_completed ):
                self.fine_positioning_steps['arm1'] >= self.fine_mpc_steps * 0.6 and 
                self.fine_positioning_steps['arm2'] >= self.fine_mpc_steps * 0.6):
            # if self.arm1_mpc_completed == True and self.arm2_mpc_completed == True:
                ...
                for attempt in range(max_attempts):
                    ...
                    if current_arm1_pos is not None and current_arm2_pos is not None:
                        ...
                        arm1_grasped = 0.02 < abs(current_arm1_pos) < 0.035  # 調整這些閾值
                        arm2_grasped = 0.02 < abs(current_arm2_pos) < 0.035
    ...
    def move_arms_to_home(self):
        ...
        try:
            ...
            home_positions_arm2 = [-0.2, 0.40, -0.8, 0.0, 0.5, -1.57]
    ...
    def reset_to_tracking_mode(self):
        ...
        # # 定義初始位置
        # initial_arm1_pos = [0.2, 0.40, -0.8, 0.0, 0.5, 1.57, -0.04, 0.04]
        # initial_arm2_pos = [-0.2, 0.40, -0.8, 0.0, 0.5, -1.57, -0.04, 0.04]

        # """重置到跟隨模式"""
        # self.get_logger().info("🔄 重置到跟隨模式")
        # self.publish_joint_commands([0.2, 0.40, -0.8, 0.0, 0.5, 1.57, -0.04, 0.04], 'arm1')
        # # 右臂
        # self.publish_joint_commands([-0.2, 0.40, -0.8, 0.0, 0.5, -1.57, -0.04, 0.04], 'arm2')
        # time.sleep(2.0)  # 等待手臂到達初始位置
        # self.get_logger().info("🟢 已讓雙臂回到初始位置")
        # self.current_mode = OperationMode.TRACKING
        # self.current_stage = HarvestStage.GLOBAL_TRACKING
        ...
        try:
            ...
            initial_positions = {
                ...
                'arm2': [-0.2, 0.40, -0.8, 0.0, 0.5, -1.57, -0.04, 0.04]
            }
            ...
            self.tracking_enabled = True
            self.current_mode = OperationMode.TRACKING
    def move_arms_to_initial_position(self):
        """讓雙臂回到初始位置"""
        # 左臂
        self.publish_joint_commands([0.2, 0.40, -0.8, 0.0, 0.5, 1.57, -0.04, 0.04], 'arm1')
        # 右臂
