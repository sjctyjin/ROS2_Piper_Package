...
class VRMPCControlNode(Node):
    def __init__(self, launch_rviz=True):
        ...
        # 關節名稱
        self.joint_names = [f"{self.arm}_joint1", f"{self.arm}_joint2", f"{self.arm}_joint3", f"{self.arm}_joint4", f"{self.arm}_joint5", f"{self.arm}_joint6", f"{self.arm}_joint7", f"{self.arm}_joint8"]
...
"""
    ...
    def publish_joint_state(self, joint_positions, gripper_value,press_A=None):
        """發布關節狀態"""
        try:
            msg = JointState()
    ...
    def control_loop(self):
        ...
        try:
            ...
            #transformations, buttons = self.vr_reader.get_transformations_and_buttons()
            if self.hand == "left":
                if 'l' not in transformations or transformations['l'] is None:
                    self.get_logger().error("❌ VR transformations['l'] 尚未取得，rotation 無效")
                    return
            else:
            ...
            # 調整坐標系
            if self.hand == "left":
                adjusted_transform = self.adjustment_matrix(transformations['l'])
            else:
            ...
            # 發布TF
            self.publish_tf(adjusted_transform, self.pose_topic)
            ...
            if a_pressed and not self.last_a_state:
                for name in self.motion_gen.kinematics.joint_names:
                    #self.get_logger().info(f"motion_gen关节名稱 {name}")
                
                    names = f"{self.arm}_{name}"
                    #self.get_logger().info(f"关节名稱 {name}")
                    #self.get_logger().info(f"關節參數 : {self.latest_joint_state.name}")
                ...
                #if res.metrics.feasible.item():
                #    next_joints = res.js_action.position.cpu().numpy()
                #    self.publish_joint_state(next_joints, self.current_gripper_value,True)
                #else:
                #    self.get_logger().warn("MPC軌跡不可行")
            ...
            if self.vr_control_active and self.reference_position is not None:
                try:
                    ...
                    target_frame= f'{self.arm}_base_link',        # ✅ 你要的目標座標
                    source_frame=self.pose_topic,    # ✅ 查詢 vr_right_hand 在 base_link 中的位置
